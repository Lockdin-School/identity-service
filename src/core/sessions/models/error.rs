...
impl ResponseError for AppError {
    ...
    fn error_response(&self) -> HttpResponse {
        ...
        match self {
            AppError::UserAlreadyExists => HttpResponse::Conflict().json(response),
            AppError::DatabaseError(_) => HttpResponse::InternalServerError().json(response),
            AppError::InternalServerError(_) => HttpResponse::InternalServerError().json(response),
            AppError::TokenGenerationError(_) => HttpResponse::InternalServerError().json(response),
            AppError::TokenExpired => HttpResponse::Unauthorized().json(response),
            AppError::InvalidToken => HttpResponse::Unauthorized().json(response),
            AppError::TokenNotFound => HttpResponse::Unauthorized().json(response),
            AppError::AuthorizationHeaderNotFound => HttpResponse::Unauthorized().json(response),
            AppError::SessionExpired => HttpResponse::Unauthorized().json(response),
        }
    }
}
